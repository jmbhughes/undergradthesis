...
\begin{figure}[ht]
  \begin{center}
    ...
    \caption{{\bf Inconsistecy for annotator} Left and right are annotations for the same image by the same annotator. As you can see the annotator \todo{detail the differences}.} 
    \label{fig:annotatorconsistency}
 \end{center}
\end{figure}

In more detail, Table \ref{tab:annotatorconsistency} tabulates which features annotators missed on average.  
...
\centering
 ...
 \hline
 \end{tabular}
 \caption{Annotator consistency: For each annotator and image, the number of inconsistenceies per label is shown.  \todo{fill in table!}}
...
This chapter describes various experiments performed on the data and the conclusions derived while working toward the optimal machine learning solar image classifier. It begins by introducing the methods of evaluation in Section \ref{sec:evaluationmethods} and then proceeds through the experiments. 
...
The Bayesian approach is most sensitive to noise levels. 
